...
class Accounts extends MX_Controller
{
  ...
  public function store_debit_voucher()
  {
    ...
    if ($this->form_validation->run()) {
      if ($this->accounts_model->insert_debitvoucher()) {
        // $this->session->set_flashdata('message', display('save_successfully'));
        // redirect('new_payment_voucher');
        echo '
        <script type="text/javascript">
        alert("Payment Voucher created successfully");
        window.location.href = "' . $base_url . 'manage_payment_voucher";
       </script>';
      } else {
        $this->session->set_flashdata('exception',  display('please_try_again'));
        redirect("new_payment_voucher");
    ...
    // } else {
      ...
      if ($this->form_validation->run()) {
          ...
          if ($this->accounts_model->insert_creditvoucher()) {
            ...
            window.location.href = "' . $base_url . 'manage_receipt_voucher";
           </script>';
          } else {
            $this->session->set_flashdata('exception',  display('please_try_again'));
            redirect("credit_voucher");
      ...
      if ($this->form_validation->run()) {
        ...
        // $crdate = date("Y-m-d");
          if ($this->accounts_model->insert_contravoucher()) {
            ...
            window.location.href = "' . $base_url . 'manage_contra_voucher";
           </script>';
      ...
      if (empty($id)) {
        if ($this->accounts_model->create_method2($bank_coa)) {
          if ($this->input->post('button', true) === "add-another") {
            ...
            window.location.href = "' . $base_url . 'add_payment_method";
           </script>';
          } else {
          echo '<script type="text/javascript">
            alert("Payment Details saved successfully");
  ...
  public function payment_receipt_type_form($id = null)
  {
    $base_url = base_url();

    $this->form_validation->set_rules('name', 'Payment/Receipt Name', 'required|max_length[200]');
    $this->form_validation->set_rules('type', 'Type', 'required|in_list[Payment,Receipt,Common]');
    ...
    if ($this->form_validation->run() === true) {
      ...
      if (empty($id)) {
        if ($this->accounts_model->insert_payment_receipt_type($postData)) {
          if ($this->input->post('button', true) === 'add-another') {
            echo '<script>alert("Payment/Receipt Type saved successfully"); window.location.href="' . $base_url . 'payment_receipt_type_form";</script>';
          } else {
            echo '<script>alert("Payment/Receipt Type saved successfully"); window.location.href="' . $base_url . 'payment_receipt_type_list";</script>';
          }
        } else {
          echo '<script>alert("Something went wrong, please try again."); window.location.href="' . $base_url . 'payment_receipt_type_form";</script>';
        }
      } else {
        $this->accounts_model->update_payment_receipt_type($id, $postData);
        echo '<script>alert("Payment/Receipt Type updated successfully"); window.location.href="' . $base_url . 'payment_receipt_type_list";</script>';
      }
    } else {
      $data['title']  = empty($id) ? 'Add Payment/Receipt Type' : 'Edit Payment/Receipt Type';
      $data['prt']    = !empty($id) ? $this->accounts_model->get_payment_receipt_type($id) : (object)['id'=>'','name'=>'','type'=>'','detail'=>'','status'=>1];
      $data['module'] = 'account';
      $data['page']   = 'payment_receipt_type_form';
      echo Modules::run('template/layout', $data);
    }
  }

  public function delete_payment_receipt_type($id = null)
  {
    $base_url = base_url();
    $productExists1 = $this->db->from('voucher_details')
    ->where('to', $id)
    ->count_all_results();

    if ($productExists1 > 0) {
      echo '<script type="text/javascript">
      alert("Cannot delete this payment receipt detail because this payment receipt detail is linked to it or something went wrong");
      window.location.href = "' . $base_url . 'payment_receipt_type_list";
     </script>';
    } else
    if ($this->accounts_model->delete_payment_receipt_type($id)) {
      echo '<script>alert("Payment/Receipt Type deleted successfully"); window.location.href="' . $base_url . 'payment_receipt_type_list";</script>';
    } else {
      echo '<script>alert("Could not delete this record."); window.location.href="' . $base_url . 'payment_receipt_type_list";</script>';
    }
  }

  public function payment_receipt_type_list()
  {
    $data['title']  = 'Payment/Receipt Type List';
    $data['module'] = 'account';
    $data['page']   = 'payment_receipt_type_list';
    echo Modules::run('template/layout', $data);
  }

  public function check_payment_receipt_type_list()
  {
    $postData = $this->input->post(null, true);
    $result   = $this->accounts_model->get_payment_receipt_type_list($postData);
    $base_url = base_url();
    $data     = [];
    $sl       = $postData['start'] + 1;

    foreach ($result['data'] as $row) {
      $editUrl   = $base_url . 'payment_receipt_type_form/' . $row->id;
      $deleteUrl = $base_url . 'delete_payment_receipt_type/' . $row->id;
      $statusBadge = $row->status_label === 'Active'
        ? '<span class="label label-success">Active</span>'
        : '<span class="label label-danger">Inactive</span>';
      $buttons = '
        <a href="' . $editUrl . '" class="btn btn-xs btn-primary"><i class="fa fa-edit"></i></a>
        <a href="' . $deleteUrl . '" class="btn btn-xs btn-danger" onclick="return confirm(\"Are you sure you want to delete this record?\")"><i class="fa fa-trash"></i></a>';
      $data[] = [
        'sl'     => $sl++,
        'id'     => $row->id,
        'name'   => $row->name,
  ...
  public function save_voucher()
  {
    ...
    $num = $this->number_generatorvoucher($this->input->post('type', TRUE),$this->input->post('type2', TRUE));

          $query = "
      INSERT INTO voucher
      (voucher_id, branch, type, `from`, date, remark, total,type2)
      VALUES
      (
      ...
      foreach ($items as $item) {
            ...
            (id, pid, `to`, sto, comment, amount,type2)
            VALUES
            (
                0,
                '{$inserted_id}',
                ...
                '{$item['sto']}',
                '{$item['comment']}',
                AES_ENCRYPT('{$item['amount']}', '{$encryption_key}'),
                AES_ENCRYPT('{$this->input->post('type2', TRUE)}', '{$encryption_key}')
            );";
         ...
         if ($this->input->post('type', TRUE) == 1) {
           $type_name = "payment_voucher";
     
         } else if ($this->input->post('type', TRUE) == 2) {
           $type_name = "receipt_voucher";
     
         } else if ($this->input->post('type', TRUE) == 3) {
           $type_name = "contra_voucher";
  ...
  public function number_generatorvoucher($type = null,$type2=null)
  {
      $encryption_key = Config::$encryption_key;

      $this->db->select_max("AES_DECRYPT(voucher_id,'" . $encryption_key . "')", 'id');
  ...
  {
      $postData = $this->input->post();
      $data = $this->accounts_model->voucher($postData, $this->input->post('type'), $this->input->post('branchid'),$this->input->post('fdate'),$this->input->post('tdate'),$this->input->post('type2'));
      echo json_encode($data);
  }
  ...
  public function create_debit_voucher()
  {
    ...
    if ($this->permission1->method('manage_payment_voucher', 'create')->access()) {
      $data['module']     = "account";
      $data['page']       = "new_voucher";
      echo Modules::run('template/layout', $data);
    } else {
      $previous_url = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : base_url();
      redirect($previous_url);
  ...
  public function create_credit_voucher()
  {
    ...
    $data['from'] = "Received To";
    $data['to'] = "Received From";
    $data['type'] = 2;
    $data['id']=0;
    ...
    // $data['crcc']       = $this->accounts_model->getCashbankHead();
    if ($this->permission1->method('manage_receipt_voucher', 'create')->access()) {
      $data['module']     = "account";
      $data['page']       = "new_voucher";
      echo Modules::run('template/layout', $data);
    } else {
  ...
  public function create_contra_voucher()
  {
    ...
    if ($this->permission1->method('manage_contra_voucher', 'create')->access()) {
      $data['module']     = "account";
      $data['page']       = "new_voucher";
      echo Modules::run('template/layout', $data);
    } else {
  ...
  public function debit_voucher()
  {
    ...
    $data['title']      = display('manage_payment_voucher');
    $data['from'] = "Paid From";
    $data['to'] = "Paid To";
    $data['type'] = 1;

    if ($this->permission1->method('manage_payment_voucher', 'read')->access()) {
      $data['module']     = "account";
      $data['page']       = "manage_voucher";
      echo Modules::run('template/layout', $data);
    } else {
      $previous_url = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : base_url();
      redirect($previous_url);
  ...
  public function contra_voucher()
  {
    ...
    $data['title']      = display('manage_contra_voucher');
    $data['from'] = "Transfered From";
    $data['to'] = "Transfered To";
    $data['type'] = 3;

    if ($this->permission1->method('manage_contra_voucher', 'read')->access()) {
      $data['module']     = "account";
      $data['page']       = "manage_voucher";
      echo Modules::run('template/layout', $data);
    } else {
  ...
  public function credit_voucher()
  {
    ...
    $data['title']      = display('manage_receipt_voucher');
    $data['from'] = "Received To";
    $data['to'] = "Received From";
    $data['type'] = 2;
    if ($this->permission1->method('manage_receipt_voucher', 'read')->access()) {
      $data['module']     = "account";
      $data['page']       = "manage_voucher";
      echo Modules::run('template/layout', $data);
    } else {
    ...
    public function edit_voucher($id = null)
    {
      ...
      // $data['crcc']               = $this->accounts_model->getCashbankHead();
      // $data['acc']                = $this->accounts_model->getTransationHead();
      $data['voucher_info']       = $this->accounts_model->get_dbvoucher_info($id );
  
      $data['all_pmethod'] = $this->pmethod_dropdown();
  
      $data['id']=$id;
      if ($data['voucher_info'][0]['type'] == 1) {
        $data['title']            = "Update Payment Voucher";
        $data['from'] = "Paid From";
        $data['to'] = "Paid To";
        $data['type'] = 1;
        $data['acc'] = $this->paymenttype_dropdown();
      }
      if ($data['voucher_info'][0]['type'] == 2) {
        $data['title']= "Update Receipt Voucher";
        $data['from'] = "Received To";
        $data['to'] = "Received From";
        $data['type'] = 2;
        $data['acc'] = $this->receipttype_dropdown();
      }
    if ($data['voucher_info'][0]['type'] == 3) {
        $data['title'] = "Update Contra Voucher";
        $data['acc']        = $this->contatype_dropdown();
        $data['from'] = "Transferred From";
        $data['to'] = "Transferred To";
        $data['type'] = 3;
  ...
  public function delete_voucher($id = null,$type = null)
  {
    date_default_timezone_set('Asia/Colombo');
    ...
    if ($type == 1) {
      $screen = "manage_payment_voucher";
      $type_name = "payment_voucher";

    } else if ($type == 2) {
      $screen = "manage_receipt_voucher";
      $type_name = "receipt_voucher";

    } else if ($type == 3) {
      $screen = "manage_contra_voucher";
      $type_name = "contra_voucher";
...
";
    ...
    echo '<script type="text/javascript">
        ...
        window.location.href = "' . $base_url . $screen . '";
      </script>';
  }
}
