...
class Accounts_model extends CI_Model
{
    ...
    public function insert_debitvoucher()
    {
        ...
        for ($i = 0; $i < count($dAID); $i++) {
            ...
            // } else if ($this->input->post('chequeno') != '') {
    ...
    public function insert_contravoucher()
    {
        ...
        for ($i = 0; $i < count($dAID); $i++) {
            ...
            $contrainsert = array(
                ...
                'COAID'          =>  $dbtid,
                'Narration'      =>  $Narration,
                'chequeNo'       =>  $this->input->post('chequeid'),
                'chequeDate'     =>  null,
                'isHonour'       =>  0,
                'ledgerComment'  =>  $Comment,
                'Debit'          =>  $Damnt,
    ...
    public function approved_vaucher($id, $action)
    {
        ...
        if ($vaucherdata) {
            foreach ($vaucherdata as $row) {
                ...
                $chequeData = $this->db->select('*')
                    ->from('cheque')
                    ->where('id', $row->chequeno)
                    ->get()->row();


                if ($chequeData !== null) {
                    if ($chequeData->depositedbank !== null) {
    ...
    public function get_dbvoucher_info($id)
    {
        ...
        return $vou_info = $this->db->select("
        v.id,
        CAST(AES_DECRYPT(v.voucher_id, '" . $encryption_key . "') AS CHAR) AS voucher_id,
        v.`from`,
        v.remark,
        v.date,
        v.branch,
        v.type,
        CAST(AES_DECRYPT(v.total, '" . $encryption_key . "') AS CHAR) AS total,
        vd.`to` AS to,
        vd.sto,
        vd.comment,
        CAST(AES_DECRYPT(vd.amount, '" . $encryption_key . "') AS CHAR) AS amount
    ")
            ->from('voucher v')
            ->join('voucher_details vd', 'vd.pid = v.id')
            ->where('v.id', $id)
            ->get()
            ->result_array();
    }
    ...
    *--------------------------*/
    ...
    public function create_method2($data = [])
    {
    ...
    {
        return $data = $this->db->select("*,IF(status = 1, 'Active', 'Inactive') as status_label")
            ->from('payment_type')
            ->where('id', $id)
            ->get()
            ->row();
    ...
    // ── Payment / Receipt Type CRUD ──────────────────────────────────────────

    public function insert_payment_receipt_type($data = [])
    {
        return $this->db->insert('payment_receipt_type', $data);
    }

    public function update_payment_receipt_type($id, $data = [])
    {
        $this->db->where('id', $id)->update('payment_receipt_type', $data);
        return $this->db->affected_rows() >= 0;
    }

    public function get_payment_receipt_type($id)
    {
    ...
    {
        $this->db->where('id', $id)->delete('payment_receipt_type');
        return $this->db->affected_rows() > 0;
    }

    public function get_payment_receipt_type_list($postData)
    {
        $columns = ['id', 'name', 'type', 'status'];
        $limit   = $postData['length'];
        $start   = $postData['start'];
        $order   = $columns[isset($postData['order'][0]['column']) ? $postData['order'][0]['column'] : 0];
        $dir     = isset($postData['order'][0]['dir']) ? $postData['order'][0]['dir'] : 'asc';

        $this->db->select("id, name, type, IF(status = 1, 'Active', 'Inactive') AS status_label");
        $this->db->from('payment_receipt_type');
        ...
        $result = $this->db->order_by($order, $dir)
            ->limit($limit, $start)
            ->get()
            ->result();

        $totalData = $this->db->count_all('payment_receipt_type');

        return ['data' => $result, 'totalFiltered' => $totalFiltered, 'totalData' => $totalData];
    }

    public function voucher($postData = null, $type, $branchid, $fdate, $tdate,$type2)
    {
        ...
        if ($searchValue != '') {
            ...
            or  AES_DECRYPT( v.voucher_id,'" . $encryption_key . "')  like '%" . $searchValue . "%' 
            OR p.name LIKE '%" . $searchValue . "%'
            OR v.date LIKE '%" . $searchValue . "%'
            OR v.remark LIKE '%" . $searchValue . "%'
        ) ";
        }
        ...
        $this->db->join('payment_type p', 'p.id=v.`from`', 'left');
        $this->db->where("AES_DECRYPT(v.type2,'" . $encryption_key . "')", $type2);
        $this->db->where('v.type', $type);

        if ($branchid > 0) {
            $this->db->where("v.branch", $branchid);
        }else{
            if ($this->session->userdata('user_level2') != 1) {
                $this->db->where_in('v.branch', $branchids);
            }
        }

        if ($fdate) {
            $this->db->where("v.date >=", $fdate);
        }

        if ($tdate) {
            $this->db->where("v.date <=", $tdate);
        }
        ...
        if($type==1||$type==2){
            ...
            (
                ...
                INNER JOIN payment_receipt_type pt 
                ON pt.id = vd.`to`
                WHERE vd.pid = v.id
            ) AS tos
        ");
        ...
        foreach ($records as $record) {
            ...
            $jsaction = "return confirm('Are You Sure ?')";
            if($type==1){
                $voucher_id="PV-".$record->voucher_id;
            }else if($type==2){
                $voucher_id="RV-".$record->voucher_id;
            }else if($type==3){
                $voucher_id="CV-".$record->voucher_id;
            }

            $permission_map = [
                1 => 'manage_payment_voucher',
                2 => 'manage_receipt_voucher',
                3 => 'manage_contra_voucher'
            ];
            
            $perm = $permission_map[$type] ?? '';

            if ($perm && $this->permission1->method($perm, 'update')->access()) {

                $button .= ' <a style="margin-left:7px;" href="' . $base_url . 'edit_voucher/' . $record->id . '" 
                class="btn btn-info btn-xs" title="' . display('update') . '">
                <i class="fa fa-pencil"></i></a>';
            }

            if ($perm && $this->permission1->method($perm, 'delete')->access()) {
                $button .= ' <a style="margin-left:7px;" href="' . $base_url . 'account/accounts/delete_voucher/' . $record->id . '/' . $type . '" 
                class="btn btn-danger btn-xs" onclick="' . $jsaction . '">
