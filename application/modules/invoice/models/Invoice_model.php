...
class Invoice_model extends CI_Model
{
    ...
    public function sale($postData = null, $type2,$branchid,$fdate,$tdate)
    {
        ...
        END AS incident,
        po.details,
        po.incidenttype,  pt.name AS paymenttype');
        $this->db->from('sale po');
        $this->db->join('customer_information si', 'si.customer_id = po.customer_id', "left");
        $this->db->join('payment_type pt', 'pt.id = po.payment_type', 'left');
        $this->db->where("AES_DECRYPT(po.type2,'" . $encryption_key . "')", $type2);
    ...
    public function quotation($postData = null, $type2,$branchid,$fdate,$tdate)
    {
        ...
        foreach ($records as $record) {
            ...
            if ($record->status != 1) {
                ...
                if ($this->permission1->method('manage_quotation', 'delete')->access()) {
                    ...
                    $button .= '  <a  style="margin-left:7px;" href="' . $base_url . 'invoice/invoice/delete_salesorder/' . $record->id . '" class="btn btn-xs btn-danger "  onclick="' . $jsaction . '"><i class="fa fa-trash"></i></a>';
                }
    ...
    public function sales_return($postData = null, $type2,$branchid,$fdate,$tdate)
    {
        ...
        $this->db->join('customer_information si', 'si.customer_id = po.customer_id', "left");
        $this->db->join('sale si1', 'si1.id = po.sales_id', "left");
        $this->db->join('payment_type pt', 'pt.id = po.payment_type', "left");
        $this->db->where("AES_DECRYPT(po.type2,'" . $encryption_key . "')", $type2);
        ...
        foreach ($records as $record) {
            ...
            if ($this->permission1->method('manage_sales_return', 'update')->access()) {
                $button .= ' <a  style="margin-left:7px;" href="' . $base_url . 'edit_sales_return/' . $record->id . '" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="left" title="' . display('update') . '"><i class="fa fa-pencil" aria-hidden="true"></i></a>';
                // $button .= ' <a  style="margin-left:7px;" href="' . $base_url . 'edit_invoice2/' . $record->id . '" class="btn btn-success btn-xs" data-toggle="tooltip" data-placement="left" title="' . display('update') . '"><i class="fa fa-pencil" aria-hidden="true"></i></a>';

            }
            if ($this->permission1->method('manage_sales_return', 'delete')->access()) {

                $button .= '  <a  style="margin-left:7px;" href="' . $base_url . 'invoice/invoice/delete_salesreturn/' . $record->id . '" class="btn btn-xs btn-danger "  onclick="' . $jsaction . '"><i class="fa fa-trash"></i></a>';
            }
