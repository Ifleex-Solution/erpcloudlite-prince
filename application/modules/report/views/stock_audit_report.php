    ...
    function generateReport() {
        $.ajax({
            ...
            success: function(data1) {
                ...
                let astock_total = datas.audit_stock.reduce((sum, item) => {
                    return sum + (parseFloat(item.astock) || 0);
                }, 0);

                let pstock_total = datas.audit_stock.reduce((sum, item) => {
                    return sum + (parseFloat(item.pstock) || 0);
                }, 0);

                const formData = new FormData();
                formData.append("aulist", JSON.stringify(datas.audit_stock));
                ...
                formData.append("astock_total", astock_total);
                formData.append("pstock_total", pstock_total);
                formData.append("astock_totalstr", convertmasterstock(astock_total, datas.audit_stock[0].conversion_ratio, datas.audit_stock[0].master, datas.audit_stock[0].sub));
                formData.append("pstock_totalstr", convertmasterstock(pstock_total, datas.audit_stock[0].conversion_ratio, datas.audit_stock[0].master, datas.audit_stock[0].sub));
                formData.append("masterunit", JSON.stringify(datas.masterunit));
                formData.append("subunit", JSON.stringify(datas.subunit));
